...
class VisualOdometry:
    ...
    def get_matches_between_two_frames(self, current_descriptors):
    ...
    def previous_current_matching(self, previous_image, current_key_points, current_descriptors,
                                  current_image_with_keypoints_drawn):
        if previous_image is not None:
            """"you can choose to visualize the points matched between every two frames by uncommenting this line"""""
            self.visualize_key_points_matching(current_descriptors, current_key_points,
                                               current_image_with_keypoints_drawn)

            # convert previous_key_points and current_key_points into floating point arrays
            array_previous_key_points = cv.KeyPoint_convert(self.previous_key_points)
            array_current_key_points = cv.KeyPoint_convert(current_key_points)
    ...
    def image_callback(self, frame):
        if self.start_feature_matching:
            try:
                ...
                self.previous_current_matching(self.previous_image, current_key_points, current_descriptors,
                                               current_image_with_keypoints_drawn)

                # starting from frame number two, calculate the matches:\
                self.get_matches_between_two_frames(current_descriptors)
